...
def findClosestImpedanceOfWires(first_wire_string: str, second_wire_string: str) -> int:
    dict1 = getDictOfPointsVisitedFromWireWithImpedance(first_wire_string)
    dict2 = getDictOfPointsVisitedFromWireWithImpedance(second_wire_string)

    return getClosestImpedancePointToZeroFromDicts(dict1, dict2)

def getClosestImpedancePointToZeroFromDicts(firstDict: dict, secondDict: dict) -> int:
    distance = 1000000000
    intersections = firstDict.keys() & secondDict.keys()
    # for x in intersections: print("Intersection: {}, steps 1: {}, steps 2: {}".format(x, firstDict[x], secondDict[x]))
    for point in intersections:
        if firstDict[point] + secondDict[point] < distance:
            distance = firstDict[point] + secondDict[point]

    return distance

def getDictOfPointsVisitedFromWireWithImpedance(wire: str) -> dict:
    currentPoint = Point(0,0)
    wire_array = wire.split(",")
    point_dict = dict()
    num_steps = 0
    for instruction in wire_array:
        ...
        for step in range(1, steps + 1):
            num_steps += 1
            nextPoint = getNextStepPoint(currentPoint, letter)
            if point_dict.get(Point) is None:
                point_dict[nextPoint] = num_steps
            currentPoint = nextPoint
    return point_dict
...
print(findClosestImpedanceOfWires(wire_1_first, wire_1_second))
print(findClosestImpedanceOfWires(wire_2_first, wire_2_second))
print(findClosestImpedanceOfWires(wire_3_first, wire_3_second))
print(findClosestImpedanceOfWires(first_full_wire, second_full_wire))
